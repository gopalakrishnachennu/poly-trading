use crate::{
    decode_command, encode_command, Error as RuntimeError, PairedCommand, PairedDecision,
    PairedRuntime,
};
...
const STREAM_ID: &str = "__paired_opportunity_runtime__";
const CHECKPOINT_MAGIC: &[u8; 8] = b"POLYPOR1";
const CHECKPOINT_VERSION: u16 = 1;
const CHECKPOINT_BODY_BYTES: usize = 56;
const CHECKPOINT_BYTES: usize = 88;

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PairedCheckpoint {
    pub sequence: u64,
...
pub struct PairedRecovery {
    pub runtime: PairedRuntime,
    pub last_sequence: Option<u64>,
}

#[derive(Debug)]
pub struct DurablePairedRuntime<J> {
    journal: J,
...
impl<J: EventJournal> DurablePairedRuntime<J> {
    /// Aligns a writer with recovered paired state.
    ///
    /// # Errors
    ///
    /// Rejects writer/recovery sequence disagreement.
    pub fn new(journal: J, recovery: PairedRecovery) -> Result<Self, StorageError> {
    ...
    /// Appends and syncs before installing one paired transition.
    ///
    /// # Errors
    ///
    /// Returns domain, schema, journal, sequence, or poisoned-owner failures.
    pub fn apply(&mut self, command: &PairedCommand) -> Result<PairedDecision, StorageError> {
    ...
    #[must_use]
    pub const fn runtime(&self) -> &PairedRuntime {
        &self.runtime
    }
...
/// Strictly replays a segmented paired-runtime journal.
///
/// # Errors
///
...
pub fn recover_segmented(
    ...
    checkpoint: Option<PairedCheckpoint>,
) -> Result<PairedRecovery, StorageError> {
...
fn validate_envelope(envelope: &EventEnvelope) -> Result<PairedCommand, StorageError> {
...
/// Creates and syncs a new paired-runtime checkpoint without replacement.
///
/// # Errors
///
...
/// Reads and verifies one exact paired-runtime checkpoint.
///
/// # Errors
///
/// Rejects length, magic, version, reserved bytes, checksum, and I/O failures.
pub fn read_checkpoint(path: impl AsRef<Path>) -> Result<PairedCheckpoint, StorageError> {
    decode_checkpoint(&fs::read(path)?)
}

fn encode_checkpoint(checkpoint: PairedCheckpoint) -> [u8; CHECKPOINT_BYTES] {
...
fn decode_checkpoint(bytes: &[u8]) -> Result<PairedCheckpoint, StorageError> {
...
pub enum StorageError {
    #[error("paired runtime error: {0}")]
    Runtime(#[from] RuntimeError),
    #[error("paired envelope error: {0:?}")]
    Schema(#[from] SchemaError),
    #[error("paired journal error: {0}")]
    Journal(#[from] JournalBackendError),
    #[error("paired segment error: {0}")]
    Segment(#[from] SegmentError),
    #[error("paired I/O error: {0}")]
    Io(#[from] std::io::Error),
    #[error("paired sequence mismatch: expected {expected}, got {actual}")]
    Sequence { expected: u64, actual: u64 },
    #[error("paired sequence is exhausted")]
    SequenceExhausted,
    #[error("paired envelope identity is invalid")]
    EnvelopeIdentity,
    #[error("paired envelope timestamp does not match command")]
    EnvelopeTimestamp,
    #[error("paired writer and recovered runtime disagree")]
    RecoveryMismatch,
    #[error("paired event follows an absorbing halt")]
    PostHaltEvent,
    #[error("paired checkpoint digest does not match its prefix")]
    CheckpointMismatch,
    #[error("paired checkpoint sequence is absent from journal")]
    CheckpointSequenceMissing,
    #[error("paired checkpoint length is invalid")]
    CheckpointLength,
    #[error("paired checkpoint magic is invalid")]
    CheckpointMagic,
    #[error("unsupported paired checkpoint version: {0}")]
    CheckpointVersion(u16),
    #[error("paired checkpoint reserved bytes are non-zero")]
    CheckpointReserved,
    #[error("paired checkpoint checksum mismatch")]
    CheckpointChecksum,
    #[error("durable paired runtime is halted: {0}")]
    Halted(String),
}
