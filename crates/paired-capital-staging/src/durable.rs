use crate::{
    decode_command, encode_command, CapitalStagingRuntime, Error as RuntimeError, StagingCommand,
    StagingDecision,
};
...
const STREAM_ID: &str = "__paired_capital_staging__";
const CHECKPOINT_MAGIC: &[u8; 8] = b"POLYPCS1";
...
pub struct StagingRecovery {
    pub runtime: CapitalStagingRuntime,
    pub last_sequence: Option<u64>,
}

#[derive(Debug)]
pub struct DurableCapitalStaging<J> {
    journal: J,
    runtime: CapitalStagingRuntime,
    last_sequence: Option<u64>,
    poisoned: Option<String>,
}

impl<J: EventJournal> DurableCapitalStaging<J> {
    /// Aligns a journal writer with recovered staging state.
    ///
    /// # Errors
    ///
    /// Rejects writer/recovery sequence disagreement.
    pub fn new(journal: J, recovery: StagingRecovery) -> Result<Self, StorageError> {
    ...
    /// Persists and syncs before installing one staging transition.
    ///
    /// # Errors
    ///
    /// Returns domain, schema, journal, sequence, or poisoned-owner failures.
    pub fn apply(&mut self, command: &StagingCommand) -> Result<StagingDecision, StorageError> {
    ...
    #[must_use]
    pub const fn runtime(&self) -> &CapitalStagingRuntime {
        &self.runtime
    }
...
/// Strictly replays a segmented capital-staging journal.
///
/// # Errors
///
...
) -> Result<StagingRecovery, StorageError> {
...
fn validate_envelope(envelope: &EventEnvelope) -> Result<StagingCommand, StorageError> {
...
/// Creates and syncs a new staging checkpoint without replacement.
///
/// # Errors
///
...
/// Reads and verifies one exact staging checkpoint.
///
/// # Errors
///
/// Rejects length, magic, version, reserved bytes, checksum, and I/O failures.
pub fn read_checkpoint(path: impl AsRef<Path>) -> Result<StagingCheckpoint, StorageError> {
    decode_checkpoint(&fs::read(path)?)
}

fn encode_checkpoint(checkpoint: StagingCheckpoint) -> [u8; CHECKPOINT_BYTES] {
...
fn decode_checkpoint(bytes: &[u8]) -> Result<StagingCheckpoint, StorageError> {
...
pub enum StorageError {
    #[error("capital staging runtime error: {0}")]
    Runtime(#[from] RuntimeError),
    #[error("capital staging envelope error: {0:?}")]
    Schema(#[from] SchemaError),
    #[error("capital staging journal error: {0}")]
    Journal(#[from] JournalBackendError),
    #[error("capital staging segment error: {0}")]
    Segment(#[from] SegmentError),
    #[error("capital staging I/O error: {0}")]
    Io(#[from] std::io::Error),
    #[error("capital staging sequence mismatch: expected {expected}, got {actual}")]
    Sequence { expected: u64, actual: u64 },
    #[error("capital staging sequence is exhausted")]
    SequenceExhausted,
    #[error("capital staging envelope identity is invalid")]
    EnvelopeIdentity,
    #[error("capital staging envelope timestamp does not match command")]
    EnvelopeTimestamp,
    #[error("capital staging writer and recovered runtime disagree")]
    RecoveryMismatch,
    #[error("capital staging event follows an absorbing halt")]
    PostHaltEvent,
    #[error("capital staging checkpoint digest does not match its prefix")]
    CheckpointMismatch,
    #[error("capital staging checkpoint sequence is absent from journal")]
    CheckpointSequenceMissing,
    #[error("capital staging checkpoint length is invalid")]
    CheckpointLength,
    #[error("capital staging checkpoint magic is invalid")]
    CheckpointMagic,
    #[error("unsupported capital staging checkpoint version: {0}")]
    CheckpointVersion(u16),
    #[error("capital staging checkpoint reserved bytes are non-zero")]
    CheckpointReserved,
    #[error("capital staging checkpoint checksum mismatch")]
    CheckpointChecksum,
    #[error("durable capital staging runtime is halted: {0}")]
    Halted(String),
}
