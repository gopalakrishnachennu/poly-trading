use crate::{
    decode_command, encode_command, Error as RuntimeError, UnifiedCommand, UnifiedOutcome,
    UnifiedPairedTradingRuntime,
};
...
const STREAM_ID: &str = "__unified_paired_trading_runtime__";
const CHECKPOINT_MAGIC: &[u8; 8] = b"POLYUPT1";
...
#[derive(Debug)]
pub struct UnifiedRecovery {
    pub runtime: UnifiedPairedTradingRuntime,
    pub last_sequence: Option<u64>,
}

#[derive(Debug)]
pub struct DurableUnifiedRuntime<J> {
    journal: J,
...
impl<J: EventJournal> DurableUnifiedRuntime<J> {
    /// Aligns a journal writer with recovered unified state.
    ///
    /// # Errors
    ///
    /// Rejects writer/recovery sequence disagreement.
    pub fn new(journal: J, recovery: UnifiedRecovery) -> Result<Self, StorageError> {
    ...
    /// Appends and syncs before installing one unified transition.
    ///
    /// # Errors
    ///
    /// Returns domain, schema, journal, sequence, or poisoned-owner failures.
    pub fn apply(&mut self, command: &UnifiedCommand) -> Result<UnifiedOutcome, StorageError> {
...
/// Strictly replays a segmented unified journal.
///
/// # Errors
///
...
pub fn recover_segmented(
    ...
    checkpoint: Option<UnifiedCheckpoint>,
) -> Result<UnifiedRecovery, StorageError> {
    let mut reader = SegmentedJournalReader::open(directory)?;
    let mut runtime = UnifiedPairedTradingRuntime::new(config)?;
...
fn validate_envelope(envelope: &EventEnvelope) -> Result<UnifiedCommand, StorageError> {
...
/// Reads and verifies one unified checkpoint.
///
/// # Errors
///
/// Rejects length, magic, version, reserved bytes, checksum, and I/O failures.
pub fn read_checkpoint(path: impl AsRef<Path>) -> Result<UnifiedCheckpoint, StorageError> {
    decode_checkpoint(&fs::read(path)?)
}

fn encode_checkpoint(checkpoint: UnifiedCheckpoint) -> [u8; CHECKPOINT_BYTES] {
...
fn decode_checkpoint(bytes: &[u8]) -> Result<UnifiedCheckpoint, StorageError> {
...
pub enum StorageError {
    #[error("unified runtime error: {0}")]
    Runtime(#[from] RuntimeError),
    #[error("unified envelope error: {0:?}")]
    Schema(#[from] SchemaError),
    #[error("unified journal error: {0}")]
    Journal(#[from] JournalBackendError),
    #[error("unified segment error: {0}")]
    Segment(#[from] SegmentError),
    #[error("unified I/O error: {0}")]
    Io(#[from] std::io::Error),
    #[error("unified sequence mismatch: expected {expected}, got {actual}")]
    Sequence { expected: u64, actual: u64 },
    #[error("unified sequence is exhausted")]
    SequenceExhausted,
    #[error("unified envelope identity is invalid")]
    EnvelopeIdentity,
    #[error("unified envelope timestamp is invalid")]
    EnvelopeTimestamp,
    #[error("unified recovery does not match the journal writer")]
    RecoveryMismatch,
    #[error("unified owner is halted: {0}")]
    Halted(String),
    #[error("unified checkpoint length is invalid")]
    CheckpointLength,
    #[error("unified checkpoint magic is invalid")]
    CheckpointMagic,
    #[error("unsupported unified checkpoint version: {0}")]
    CheckpointVersion(u16),
    #[error("unified checkpoint reserved bytes are non-zero")]
    CheckpointReserved,
    #[error("unified checkpoint checksum is invalid")]
    CheckpointChecksum,
    #[error("unified checkpoint digest does not match replay")]
    CheckpointMismatch,
    #[error("unified checkpoint sequence was not present")]
    CheckpointSequenceMissing,
    #[error("unified journal contains an event after terminal halt")]
    PostHaltEvent,
}
