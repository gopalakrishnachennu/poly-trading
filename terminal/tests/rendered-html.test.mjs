...
test("server render fails closed before the public gateway is available", async () => {
  ...
  // Default MARKETS workspace must render fail-closed.  A later temporary
  // outage may retain an already-validated frame as clearly stale context,
  // but the boot shell has neither identity nor execution authority.
  assert.match(html, /STALE CONTEXT — NOT AUTHORIZED FOR PAPER DECISIONS/);
